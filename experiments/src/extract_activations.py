...
def main():
    ...
    parser.add_argument("--mode", required=True, choices=["training", "test", "counterfactuals"])
    parser.add_argument("--category", choices=TEST_CATEGORIES)
    parser.add_argument("--dataset_name", help="Dataset name (e.g. ssv2 or k400) for counterfactuals mode")
    ...
    elif args.mode == "counterfactuals":
        if not args.dataset_name:
            parser.error("--dataset_name required when mode=counterfactuals")
        metadata_path = DATA_DIR / f"counterfactual_metadata_{args.dataset_name}.csv"
        output_dir = ACTIVATIONS_DIR / f"counterfactuals_{args.dataset_name}_{args.model}"
    else:
